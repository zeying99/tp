...
This command allows users of the AddressBook application to add optional remarks to people in their definition book and edit it if required. The command should have the following format:
...
Let’s start by creating a new `RemarkCommand` class in the `src/main/java/seedu/definition/logic/command` directory.
...
``` java
package seedu.definition.logic.commands;

import seedu.definition.model.Model;

/**
<<<<<<< HEAD
 * Changes the remark of an existing person in the definition book.
=======
...
``` java
import static seedu.definition.commons.util.CollectionUtil.requireAllNonNull;
//...
public class RemarkCommand extends Command {
    //...
...
Create a `RemarkCommandParser` class in the `seedu.definition.logic.parser` package. The class must extend the `Parser` interface.
...
<<<<<<< HEAD
Create a new `Remark` in `seedu.definition.model.person`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
=======
...
Simply add the following to [`seedu.definition.ui.PersonCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
...
:exclamation: You must delete AddressBook’s storage file located at `/data/addressbook.json` before running it! Not doing so will cause AddressBook to default to an empty definition book!
